...
export interface IAgentSignupSession {
  sessionId: string;
  method: SignupMethod;
  status: SignupStatus;
...
export interface IAgentSignupSessionDocument
  extends IAgentSignupSession,
    Document {
  _id: Types.ObjectId;
  createdAt: Date;
  updatedAt: Date;
...
const agentSignupSessionSchema = new Schema<IAgentSignupSessionDocument>(
  {
...
agentSignupSessionSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });

export const AgentSignupSession: Model<IAgentSignupSessionDocument> =
  mongoose.models.AgentSignupSession ||
  mongoose.model<IAgentSignupSessionDocument>(
    'AgentSignupSession',
    agentSignupSessionSchema
  );
